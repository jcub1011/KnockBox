...
{
    public partial class FinalGuessPhase : ComponentBase
    {
        ...
        private bool HasAlreadyGuessed => UserService.CurrentUser != null && GameState.GamePlayers.TryGetValue(UserService.CurrentUser.Id, out var p) && p.HasSubmittedGuess;
        ...
        {
            if (UserService.CurrentUser == null) return;
            var result = Engine.SkipFinalGuess(UserService.CurrentUser, GameState);
            if (result.IsFailure && result.TryGetFailure(out var err))
                ShowError(err.PublicMessage);
        }
    }
