...
- `KnockBox.Plugins.Analyzer` ships KB1001–KB1004 Roslyn analyzers that flag sandbox-escape APIs (filesystem, HTTP, process, environment) at compile time. **First-party plugins in this repo do not currently reference it** — it ships with the public `dotnet new knockbox-game` template (`KnockBox.Templates`) for third-party authors.
