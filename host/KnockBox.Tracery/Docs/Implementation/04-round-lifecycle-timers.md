...
- **`EnterRoundIntro`** — if `CurrentRound >= TotalRounds`, go to `EnterFinalStandings`; else set `Phase = RoundIntro`, `PhaseExpiresAtUtc = now + TransitionDuration`, kick off the next board's generation on a worker thread, and schedule the commit (`EnterPlaying`).
- **`GenerateRoundBoard(settings)` / `EnterPlaying`** — generation runs *off the execute lock* during the intro window (it does up to ~50 DFS solves; holding the write lock for that would starve `SubmitTrace`/`SkipReveal`). The scheduled commit awaits the in-flight board, then `EnterPlaying` stores `CurrentGrid` + the two findable sets, resets each player's round bank, sets `IsRoundActive = true`, `Phase = Playing`. If `RoundTimer > 0`, set `PhaseExpiresAtUtc` and `ScheduleCallback(RoundTimer, EndRoundIfStillActive(capturedRound))`; else unlimited (`PhaseExpiresAtUtc = null`) — host-advanced.
...
- **`AdvanceAfterResults`** — when the reveal runs its full duration the players have already seen the results screen and its next-round indicator, so it advances **straight into the next round** (`EnterPlaying`) — the intermissions are deliberately collapsed (no extra "get ready" hop). When done (`CurrentRound >= TotalRounds`), `EnterFinalStandings`. A host who *skips* the reveal early instead routes through `EnterRoundIntro` (`SkipReveal`), since the cut-short reveal didn't give players the full transition — they get the get-ready countdown to catch up.
