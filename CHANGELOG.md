...
**`AbstractGameEngine`**
- `StartAsync(User caller, AbstractGameState state, CancellationToken ct = default)` is now the **non-virtual public entry point** on the base class. It verifies `caller.Id == state.Host.Id` and then delegates to a new abstract `protected Task<Result> StartAsyncCore(AbstractGameState state, CancellationToken ct = default)`. Plugins override `StartAsyncCore`, not `StartAsync` — `override` is impossible because the base is not `virtual`, and shadowing via `new` in a derived class is an authoring violation that defeats the host-identity check and is not supported. Host-identity authorization is therefore enforced once, in the platform, rather than duplicated across every plugin's lobby page. Migration: rename `override StartAsync(AbstractGameState, ...)` → `override StartAsyncCore(AbstractGameState, ...)`; drop any in-engine `host != state.Host` check (the base class now handles it).
