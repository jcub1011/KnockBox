...
KnockBox supports two kinds of plugin: **game plugins** (user-facing, with a home-page tile and a route) and **library plugins** (non-user-facing, expose shared services like a word dictionary that multiple games can consume).

- The host `KnockBox.csproj` references both kinds of plugin with `ReferenceOutputAssembly="false" Private="false"`. Those references exist **only** to make the plugins build transitively — the host takes no compile-time dependency on plugin types.
- Each plugin `.csproj` imports either [`Directory.Plugin.targets`](host/Directory.Plugin.targets) (games, staged to `games/{PluginName}/`) or [`Directory.Library.targets`](host/Directory.Library.targets) (libraries, staged to `libraries/{PluginName}/`). The targets copy the plugin DLL, `.deps.json`, `wwwroot/`, and — for libraries — the sibling contracts DLL into the host's bin folder.
- At startup, `AddKnockBoxPlatform` calls `PluginLoader.LoadModules(...)` with both root paths. Each plugin is loaded into its own `PluginLoadContext` (AssemblyLoadContext) rooted at its folder. `KnockBox.Core`, the BCL, logging/DI abstractions, **and library plugins' `exportedContracts` DLLs** are loaded into the default ALC so type identity is preserved across the host/plugin boundary. Library plugins are always loaded *before* any game plugin so game-side constructor injection of library-exported services resolves cleanly.
- Each plugin exposes **exactly one** module type with a public parameterless constructor — `IGameModule` for a game (calls `AddGameEngine<TEngine>()` exactly once in `RegisterServices`) or `ILibraryModule` for a library (just calls `AddSingleton<TContract, TImpl>()` for the services it exports; no engine). `Program.cs` maps each game plugin's `wwwroot/` folder to `/_content/{PluginName}` and registers each plugin's assembly with Blazor routing so its `@page` components are reachable.
- Library plugins use strict `Major.Minor.Patch` SemVer. Same Major.Minor with higher Patch wins; different Major or Minor coexist side-by-side (e.g. `libraries/KnockBox.WordService.v1.2/` and `libraries/KnockBox.WordService.v1.3/`).

The first library plugin in the repo is `host/KnockBox.WordService` (with sibling contracts `host/KnockBox.WordService.Contracts`); Spar-dle is its first consumer. See [`host/KnockBox/Specs/knockbox-platform-architecture.md`](host/KnockBox/Specs/knockbox-platform-architecture.md) for the full design, including the library/game two-pass DI registration order, service shadowing rules, and the multi-version coexistence policy.
